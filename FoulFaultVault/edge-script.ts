...
BunnySDK.net.http.serve(async (request: Request): Promise<Response> => {
  ...
  // Gemini endpoint – no mock, returns real errors
  if (path === "/gemini" && request.method === "POST") {
    ...
    // Use gemini-2.5-flash (as you confirmed works)
    ...
    // Return the actual error (no mock)
    if (geminiData.error || !geminiRes.ok) {
      return new Response(JSON.stringify({ error: geminiData.error || { message: "Gemini API error", status: geminiRes.status } }), {
